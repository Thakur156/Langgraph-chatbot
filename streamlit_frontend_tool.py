            ...
            for message_chunk, metadata in Chatbot.stream(
                ...
                if node_name == "tools":
                    ...
                    if tool_name and tool_name not in used_tools:
                        ...
                        tool_status.write(f"Using tool: `{tool_name}`")
                        tool_status.update(label=f"Using tool: {tool_name}", state="running")
                    continue

                if node_name == "chat_node" and message_chunk.content:
                    yield message_chunk.content

            if used_tools:
                tool_status.update(label="Response ready", state="complete", expanded=False)
            else:
                tool_status.update(label="No tool used", state="complete", expanded=False)
